...
def plot(inp_file, filtered_data, output_file):
    ...
    plt.subplot(1, 2, 2)
    plt.title("Filtered")
    plt.plot(inp_file[:, 0], filtered_data)
    plt.xlabel("Energy")
    plt.ylabel("DOS")
    plt.show()

def filter_data(inp_file, kernel):
    filtered_data = np.column_stack((inp_file[:, 0], convolve1d(inp_file[:, 1], kernel)))
...
def main():
    ...
    plot(inp_file, filtered_data[:, 1], args.outfile)
